...
- Notifications and detailed records can use different cumulative origins. Detailed records expose thread and turn counters; a notification may restart at a turn boundary or continue across turns. Exact matches to those explicit counters establish a persisted offset before notification subtraction and identity handling. A new turn alone never resets that offset. Without a proven notification origin, the established thread baseline is preserved; later detailed cumulative progress includes intervening usage. An unmatched notification tail remains deferred until the origin or detailed progress is available. Already covered detailed records cannot change origin metadata before deduplication. Repeated covered notifications cannot rewind the baseline even after new assistant output. Covered delayed details add identity links without adding quantities. Notification progress is tracked separately so a delayed notification cannot rewind a detailed interval. A notification's own regression establishes a new segment; a turn/settings/abort boundary alone does not establish usage. New assistant output does not make an already covered detailed record a new charge.
...
## Automatic historical accounting revision

The `codex-counter-origins-v1` revision checks the application's existing Codex sources, without a user-specific date range. Unlike a price revision, it can correct quantities and duplicate execution ownership. It uses the current production scanner plus a narrow evidence matcher; no old parser or compatibility shape is shipped.

A correction must match a known defect equation in the frozen raw prefix: a structured record was charged as thread total minus an unrelated notification total, while its actual usage equals the difference between consecutive thread totals and the previous notification maps exactly to that previous thread total using an explicitly witnessed origin; or a repeated notification in the same turn was charged again for an identified retained execution. The retained response identity, token components, current canonical record, and every affected hour/day/month bucket (including categories and costs) must reconcile. New-versus-old replay differences alone do not authorize a change. Unexplained differences remain recorded as unverified.

Before the first quantity correction in a run, a validated SQLite backup is saved in `usage-accounting-backups`. Each source commits its corrected entries, bucket deltas, execution fingerprints/aliases, and completion report together. Stable retained request IDs are kept; a proven duplicate's aliases move to its surviving execution. Same-source protected seeds without quantities can be promoted only when the original response identity proves their retained owner. Foreign identities remain protected. Source checkpoints, source attribution, session metadata, and unaffected history are preserved. Checkpoint, retained rows, buckets, identities, and the raw prefix are rechecked before committing.

The revision snapshots the existing source manifest once in `usage_accounting_revision` and `usage_accounting_revision_source`, using the current schema 6 database. Newly collected sources use the fixed parser and do not repeatedly reopen this migration. A crash leaves completed source transactions intact and pending sources resumable. Completed sources are not charged or corrected again. Missing-file sources are rechecked automatically when discovery finds them again; other preserved sources are retried only through the explicit recheck action. Rechecking a partially corrected source retains its previous correction totals. Reset deletes source receipts through their source foreign keys.

Collection and compaction wait behind the same exclusive refresh queue while the worker checks history. The window remains responsive and shows progress. Finishing (or failing) refreshes only local usage projections, without requesting provider quotas or resetting the index. The main view shows one dismissible result; Settings → Data retains details and a recheck entry. Reports distinguish corrected, unchanged, preserved, and partially corrected sources. Dismissal persists against the result signature, so unchanged results do not repeatedly notify. Missing request detail, rewritten/unavailable sources, conflicting ownership, and incomplete buckets retain their unverified amounts. Their original accuracy is not asserted. The eight-day request-detail retention policy is unchanged.

For explicit maintenance, `npm run reconcile:usage-accounting -- --database=<path> --report=<path>` previews the same revision without writing the database. Add `--apply` to apply certified corrections, `--backup-directory=<path>` to select the backup location, and `--retry-preserved` to recheck unresolved sources. The app must be closed for CLI application; concurrent changes otherwise cause the affected source to be preserved. Reports contain local opaque source labels and verification reasons, never raw conversation text. Backups remain local and are not uploaded.

## Historical continuity and migration

Normal refresh preserves stored history. A missing source does not delete its totals. File truncation or prefix change freezes the current readable end, reconstructs parser state without charging that prefix again, then continues from subsequent appends. Obsolete Codex accounting state is discarded as a whole rather than read through a compatibility adapter. Parser upgrades likewise reconstruct context through the committed boundary without replacing historical aggregates. Physical generations distinguish genuine later usage from pre-rewrite counter values.
...
Codex parser 7 / accounting state 4 keeps a cumulative baseline, notification progress and origin, the latest explicit thread/turn counters, generation/segment, and at most 128 recent observation keys. Claude retains at most 128 recent applied requests and 256 blocks per retained request. Persistent execution identities grow with distinct observed executions; they are not an in-memory history-matching buffer.
