...
// Refresh must stay passive: callers should only invoke refreshNow after the
// usage API returns 401. Any future preflight path must fall back to the
// current access token when refresh does not return ok.
...
export function shouldPreflightRefresh(): boolean {
  return getOAuthCredentialState().shouldRefresh;
}
...
function normalizeCooldown(value: unknown): OAuthRefreshCooldown | null {
  ...
  let normalizedUntil = until;
  let consecutiveCount = Math.max(0, Math.floor(rawCount));
  let credentialMarker = typeof record.credentialMarker === 'string' && record.credentialMarker.length > 0
    ? record.credentialMarker
    : undefined;
  if (!credentialMarker && record.reason === 'http-429' && consecutiveCount > 1) {
    const firstStepMs = Math.max(retryAfterMs ?? 0, RATE_LIMIT_BACKOFF_LADDER_MIN[0] * 60_000);
    normalizedUntil = Math.min(until, recordedAt + firstStepMs);
    consecutiveCount = 1;
    credentialMarker = currentCredentialMarker() ?? undefined;
  }
  return {
...
function readPersistedCooldown(): OAuthRefreshCooldown | null {
  ...
  try {
    const raw = refreshStore.get(OAUTH_REFRESH_COOLDOWN_KEY);
    const normalized = normalizeCooldown(raw);
    if (!normalized) {
      try {
        refreshStore.delete(OAUTH_REFRESH_COOLDOWN_KEY);
      } catch {
        // Ignore store cleanup failures; refresh logic still has memory fallback.
      }
    } else if (!cooldownMatchesCurrentCredential(normalized)) {
      try {
        refreshStore.delete(OAUTH_REFRESH_COOLDOWN_KEY);
      } catch {
        // Ignore store cleanup failures; refresh can proceed using the current credentials.
      }
      return null;
    } else {
      ...
      if (
        record
        && (record.until !== normalized.until
          || record.consecutiveCount !== normalized.consecutiveCount
          || record.credentialMarker !== normalized.credentialMarker)
      ) {
        try {
          refreshStore.set(OAUTH_REFRESH_COOLDOWN_KEY, normalized);
        } catch {
          // Keep using the normalized in-memory value even if persistence fails.
        }
      }
    }
    return normalized;
...
export function getOAuthCredentialState(): OAuthCredentialState {
  ...
  const accessToken = cred?.claudeAiOauth?.accessToken;
...
export function getOAuthCredentialFileState(): OAuthCredentialFileState {
  ...
  try {
    ...
    // Credential parsing above remains the source of truth; stat is only a retry marker.
  }
...
export async function refreshNow(userAgent: string, _trigger = 'unknown'): Promise<RefreshOutcome> {
  ...
  if (cooldownBefore && now < cooldownBefore.until) {
    const outcome = rateLimitedFromCooldown(cooldownBefore, now);
    return outcome;
  }
  if (inflight) {
...
async function doRefresh(userAgent: string): Promise<RefreshAttemptResult> {
  ...
  if (resp.status === 400 || resp.status === 401) {
    const serverMessage = refreshErrorMessage(resp.body);
    return { outcome: { kind: 'invalid-grant', serverMessage }, networkAttempted: true, httpStatus: resp.status };
  }

  if (resp.status === 429) {
    const serverMessage = refreshErrorMessage(resp.body);
    return {
      outcome: recordHttp429Cooldown(serverMessage, retryAfterMsFromHeader(resp.headers?.['retry-after'])),
  ...
  if (resp.status < 200 || resp.status >= 300) {
    return { outcome: { kind: 'unexpected', status: resp.status, body: resp.body.slice(0, 500) }, networkAttempted: true, httpStatus: resp.status };
  }
...
async function postForm(url: string, params: Record<string, string>, userAgent: string): Promise<{ status: number; body: string; headers?: Record<string, string | string[] | undefined> }> {
